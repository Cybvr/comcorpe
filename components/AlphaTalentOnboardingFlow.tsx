...
export default function AlphaTalentOnboardingFlow() {
  ...
  function canAdvance() {
    const emailValue = form.email.trim() || currentUser?.email?.trim()
    if (step === 1) return form.fullName.trim() && emailValue && form.role.trim() && form.industry && form.location && form.yearsExp
  ...
  async function next() {
    ...
    const normalizedEmail = (form.email || currentUser?.email || '').toLowerCase().trim()

    if (typeof window !== 'undefined') {
      localStorage.setItem('alpha_talent_onboarding', JSON.stringify({ ...form, email: normalizedEmail, completedAt: new Date().toISOString() }))
    }
      ...
      <div className="w-full max-w-[520px]">
        ...
        {step === 0 && (
          <div className="space-y-8">
            <div>
              <p className="mb-2 font-mono text-[10px] uppercase tracking-eyebrow text-primary">Welcome</p>
              <h1 className="font-display text-[32px] font-black leading-tight tracking-[-0.03em] text-foreground">Before we begin</h1>
              <p className="mt-2 font-text text-sm text-muted-foreground">A quick word from the Comcorpe team before you build out your alpha talent profile.</p>
            </div>
        ...
        {step === 1 && (
          <div className="space-y-6">
            <div>
              ...
              <p className="mt-2 font-text text-sm text-muted-foreground">Tell us who you are and where we should reach you. This is your alpha talent profile.</p>
            </div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2">
                ...
                <input className={INPUT} type="email" placeholder="you@example.com" value={form.email || currentUser?.email || ''} onChange={(event) => set('email', event.target.value)} autoComplete="email" />
              </div>
              <div className="col-span-2">
        ...
        <div className="mt-10 flex items-center justify-between">
          ...
          {step === 0 ? (
            <button
              onClick={() => setStep(1)}
              className="flex items-center gap-2 bg-foreground px-7 py-3 font-text text-sm font-semibold text-background transition-colors duration-100 hover:bg-primary hover:text-primary-foreground"
            >
