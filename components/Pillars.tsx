...
export default function Pillars() {
  ...
  return (
    <section id="model" className="py-20 md:py-32 px-6 md:px-24 bg-foreground text-background border-b border-foreground">
      <div className="flex items-baseline gap-6 mb-14">
        ...
        <span className="font-mono text-xs text-background/60 ml-auto">03 / 09</span>
      </div>
      ...
      <div className="flex flex-col md:grid md:grid-cols-2 gap-12 md:gap-16 mb-20 md:mb-24 md:items-end">
        ...
        <div className="font-text text-[18px] leading-relaxed text-background/70 max-w-[38ch]">
          <span className="font-display font-black text-[22px] tracking-[-0.02em] text-background block mb-3">Three pillars hold the model.</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 border border-background/20">
        {pillars.map((p, i) => (
          <div
            key={p.i}
            className={`group p-8 md:p-12 hover:bg-primary/[0.08] transition-colors duration-[240ms] cursor-default min-h-[400px] md:min-h-[460px] flex flex-col ${i < pillars.length - 1 ? 'border-b md:border-b-0 md:border-r border-background/20' : ''}`}
          >
            <div className="flex items-baseline gap-3 mb-16">
              <span className="font-mono text-[13px] text-background/60">{p.i}.</span>
            </div>
            <div className="font-display font-black text-[64px] leading-[0.92] tracking-hero text-background">
      ...
      <div className="mt-20 md:mt-24 border border-background/20 grid grid-cols-1 md:grid-cols-[180px_1fr_1fr] overflow-x-auto">
        <div className="p-5 border-b border-r border-background/20 font-mono text-xs text-background/60 uppercase tracking-[0.08em]">vs.</div>
        <div className="p-5 border-b border-r border-background/20 font-mono text-xs text-background/60 uppercase tracking-[0.08em]">Traditional Model</div>
        <div className="p-5 border-b border-background/20 font-mono text-xs text-white uppercase tracking-[0.08em] bg-primary">Comcorpᵉ Model</div>
        {rows.map((r, i) => (
          <Fragment key={i}>
            <div key={`l-${r.label}`} className={`px-6 py-6 font-mono text-xs text-background/60 uppercase tracking-[0.08em] border-r border-background/20 ${i < rows.length - 1 ? 'border-b border-background/[0.12]' : ''}`}>{r.label}</div>
            <div key={`a-${r.label}`} className={`px-6 py-6 font-text text-[17px] text-background/85 border-r border-background/20 ${i < rows.length - 1 ? 'border-b border-background/[0.12]' : ''}`}>{r.a}</div>
            <div key={`b-${r.label}`} className={`px-6 py-6 font-text text-[17px] text-background font-medium bg-primary/[0.08] ${i < rows.length - 1 ? 'border-b border-background/[0.12]' : ''}`}>{r.b}</div>
          </Fragment>
        ))}
      </div>
    </section>
