    ...
    <div className="px-4 py-6 sm:px-6 lg:px-8 lg:py-8 max-w-[860px] mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-[auto_1fr] gap-8 items-start">
        <div className="flex flex-col items-center gap-3">
          <div className="w-20 h-20 bg-ink flex items-center justify-center font-display font-black text-[24px] text-paper">
            {currentUser.initials}
          </div>
          <span className="font-mono text-[10px] uppercase tracking-eyebrow text-ink-40 border border-ink-20 px-2 py-0.5">
            {currentUser.role}
          </span>
        </div>
        ...
        <div className="space-y-6">
          {editing ? (
            <div className="space-y-4">
              <div className="flex flex-col gap-1.5">
                <label className="font-mono text-[11px] tracking-eyebrow uppercase text-ink-60">Contact name</label>
                <input className={I} value={name} onChange={e => setName(e.target.value)} />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="font-mono text-[11px] tracking-eyebrow uppercase text-ink-60">Company</label>
            ...
            <div className="space-y-1">
              ...
              <p className="font-text text-sm text-ink-60">{company}</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[
              ...
              <div key={row.label} className="flex items-start gap-3 px-4 py-3 border border-ink-10 bg-paper">
                <row.icon size={14} strokeWidth={1.5} className="text-ink-40 mt-0.5 shrink-0" />
                <div>
                  <p className="font-mono text-[10px] uppercase tracking-eyebrow text-ink-40">{row.label}</p>
                  <p className="font-text text-sm text-ink font-semibold mt-0.5">{row.value}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="mt-10">
        <h3 className="font-display font-black text-[18px] tracking-[-0.02em] text-ink mb-4">Engagement history</h3>
          ...
          {clientJobs.map(job => (
            <div key={job.id} className="px-5 py-4 flex items-center justify-between gap-4">
              <div className="min-w-0">
                ...
                <p className="font-mono text-[10px] text-ink-40 mt-0.5">{job.type} · {job.rate}</p>
              </div>
              <span className={`shrink-0 font-mono text-[10px] tracking-eyebrow uppercase px-2 py-0.5 ${
                job.status === 'Active' ? 'bg-green-100 text-green-700'
                : job.status === 'Completed' ? 'bg-ink-10 text-ink-60'
