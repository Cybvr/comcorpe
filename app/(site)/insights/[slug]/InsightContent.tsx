...
import { useInsightBySlug } from '@/lib/insights'

function SourceRefs({ refs }: { refs: number[] }) {
  return (
    <sup className="ml-2 inline-flex gap-1 align-super">
      {refs.map(ref => (
        <a key={ref} href={`#source-${ref}`} className="font-mono text-[10px] text-primary hover:underline">
          [{ref}]
        </a>
      ))}
    </sup>
...
export default function InsightContent({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="bg-background">
      ...
      {insight.thesis && insight.signals && (
