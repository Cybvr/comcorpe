        ...
        <div className="mb-16 md:mb-24 border-b border-ink pb-14 md:pb-20">
          ...
          <div className="flex flex-col md:flex-row md:items-end gap-8 md:gap-24">
            <h1 className="font-display font-black text-[clamp(48px,7vw,96px)] leading-[0.92] tracking-hero text-ink m-0 text-balance">
              ...
              <span className="text-blue">coverage.</span>
            </h1>
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-2 gap-16 lg:gap-24">
          ...
          <div className="lg:sticky lg:top-32 h-fit">
            ...
            <div className="flex flex-col border-t border-ink">
              {rungs.map((r, i) => (
                <div key={r.n} className="grid grid-cols-[40px_1fr_auto] gap-5 py-7 border-b border-ink-10 items-baseline">
                  <span className="font-mono text-xs text-ink-40">{r.n}</span>
                  <div>
                    <div className="font-display font-black text-[24px] tracking-[-0.02em] text-ink">{r.l}</div>
                    <div className="font-text text-[16px] text-ink-60 mt-1.5">{r.d}</div>
                  </div>
                  <span className="font-mono text-[11px] text-blue uppercase tracking-[0.08em] border border-blue px-2 py-1 h-fit">{r.r}</span>
                </div>
              ))}
            </div>

            <div className="mt-12 p-8 bg-ink text-paper grid grid-cols-[1fr_auto] items-center gap-8">
              <div>
                <div className="font-mono text-[11px] tracking-[0.08em] uppercase text-paper/60 mb-2">Primary Region</div>
                <div className="font-display font-black text-[24px] tracking-[-0.02em] text-paper">Pan-Africa</div>
              </div>
              <div className="font-mono text-[12px] text-paper/60 flex items-center gap-2">
                <span className="text-blue font-bold text-xl">→</span> 
