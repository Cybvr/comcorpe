...
export default function TalentJobsPage() {
  ...
  const displayJobs = filter === 'open' ? jobs.filter(j => ['Scoping', 'Pod review'].includes(j.status)) : jobs
    ...
    <div className="px-8 py-8 max-w-[1240px] mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-4">
            <h1 className="font-display font-black text-[32px] tracking-[-0.03em] text-ink leading-none">Jobs</h1>
            ...
            <div className="inline-flex p-1 bg-ink-5 rounded-lg w-fit">
              ...
              <button
                ...
                className={`px-4 py-1.5 rounded-md font-text text-xs font-semibold transition-all flex items-center gap-2 ${
                  filter === 'open' ? 'bg-paper text-blue shadow-sm' : 'text-ink-40 hover:text-ink'
                }`}
              >
                Open
                {jobs.filter(j => ['Scoping', 'Pod review'].includes(j.status)).length > 0 && (
                  <span className={`w-1.5 h-1.5 rounded-full ${filter === 'open' ? 'bg-blue' : 'bg-ink-20'}`} />
                )}
              </button>
            </div>
      ...
      {displayJobs.length === 0 && (
        <div className="py-20 text-center border-2 border-dashed border-ink-10 rounded-2xl">
          <p className="font-text text-ink-40">No {filter} matches found.</p>
        </div>
      )}
    </div>
  )
